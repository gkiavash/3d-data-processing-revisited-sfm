...
void FeatureMatcher::extractFeatures()
{
  ...
  for( int i = 0; i < images_names_.size(); i++  )
  {
    ...
    /////////////////////////////////////////////////////////////////////////////////////////
      ...
      // cv::Ptr<cv::Feature2D> detc = cv::ORB::create(MAX_FEATURES);
      cv::Ptr<cv::Feature2D> detc = cv::SIFT::create();
      // cv::Ptr<cv::xfeatures2d::SURF> detc = cv::xfeatures2d::SURF::create( 400 );

      detc->detectAndCompute(imgGray, cv::Mat(), keypoints_new, descriptors_new);


      auto itPos_f = features_.begin() + i;
      features_.insert(itPos_f, keypoints_new);

      auto itPos_d = descriptors_.begin() + i;
      descriptors_.insert(itPos_d, descriptors_new);

      std::cout << "\n size: " << keypoints_new.size() << " , " << descriptors_new.size();


      std::vector<cv::Vec3b> feats_colors_new;

      for(int i=0; i<img.rows; i++)
          for(int j=0; j<img.cols; j++)
              feats_colors_new.push_back(img.at<cv::Vec3b>(i,j));

      auto itPos_c = feats_colors_.begin() + i;
      feats_colors_.insert(itPos_c, feats_colors_new);
...
void FeatureMatcher::exhaustiveMatching()
{
  for( int i = 0; i < images_names_.size() - 1; i++ )
  {
    for( int j = i + 1; j < images_names_.size(); j++ )
    {
      ...
      /////////////////////////////////////////////////////////////////////////////////////////
      // bin/matcher ../datasets/3dp_cam.yml ../datasets/images_1/ output/
      cv::BFMatcher matcher(cv::NORM_L2,true);
      matcher.match( descriptors_[i], descriptors_[j], matches );

//        cv::Ptr<cv::DescriptorMatcher> matcher = cv::DescriptorMatcher::create("BruteForce-Hamming");
//        matcher->match(descriptors_[i], descriptors_[j], matches, cv::Mat());
